...
Income and education (pct_bachelors_higher) appear most positively correlated with turnout, meaning counties with higher income and more college graduates tend to vote at higher rates. Unemployment shows a negative correlation — higher unemployment links to lower turnout. Median age has a modest positive relationship with turnout as well. Population and pct_white appear weakly or not correlated with turnout at all, as there's no clear pattern between those variables and voting rates.

In the scatterplots, income and education show the clearest linear relationships with turnout. Unemployment shows a negative linear trend. Population shows little to no linear relationship, appearing mostly scattered.
...
The model explains about 80% of the variation in voter turnout, which is strong. The residuals are small and close to zero, meaning predictions are generally accurate.

Income, median age, and pct_white are the only statistically significant predictors. Population, education, and unemployment are not significant, meaning they don't add much to the model.

Overall this is a useful model — it explains most of the variation in turnout and is statistically significant as a whole. Removing the insignificant variables could make it even better.
...
Turnout and pct_white have the most significance compared to when turnout is compared to income and population indivdually. 
...
# Model 3
model3 <- lm(turnout ~ pct_white, data = md_turnout)
summary(model3)
...
This analysis could help identify which Maryland counties are voting at higher or lower rates than expected given their demographics. For example, a wealthy, educated county with surprisingly low turnout could be a story worth investigating — are there registration barriers or polling access issues? The analysis also helps reporters explain why turnout differs across communities, rather than just reporting the raw numbers.
