...
```{r}
model <- lm(Wins ~ `Yards Allowed` + Takeaways + Giveaways, data = teams)
summary(model)
```
...
This model is useful, but not the best we can find with the provided data. There is a better combination of the data from the data we have accessible to us.
...
Points scored have the most impact on wins. While not a complete determinant, scoring more points usually leads to more wins on the season. Yards gained is not impactful at all becasue an offense could consistently go the entire length of the field but not score, leading to more losses.
...
This is a model that tells us how correlated certain statistics are with one another. For example, it tells us that while takeaways and points scored don't really have an impact on one another, points scored and yards gained do. The more correlated the two variables, the better it is for the team.
